# PACKAGE DESCRIPTION
- src/thread/control/printer : interrupt() 사용하는 실용 예제 코드 
# NOTE
## 0. 개념
### 멀티태스킹과 멀티프로세싱
...
### Interrupt
- 인터럽트를 사용하면, `WAITING`, `TIMED_WAITING` 같은 대기 상태의 스레드를 바로 깨워서 RUNNABLE 상태로 만든다
- 특정 스레드의 인스턴스에 interrupt() 메서드를 호출하면, 해당 스레드에 인터럽트가 발생한다.
  1. 이때, 인터럽트를 받은 스레드는 **대기(sleep) 상태에서 깨어나** RUNNABLE 상태가 되고, 코드를 수행한다.
  2. 인터럽트가 발생하면 해당 스레드에 `interruptedException`이 발생한다.
  3. 예외가 던져 지고 나면, 정상적인 Runnable 상태가 되며 스레드는 코드를 계속 실행 시킨다.
     - `interrupt()`를 호출한다고 즉각 `interruptedException`를 발생시키는 것은 아니고,
     - `sleep()`처럼 `interruptedException`를 던지는 메서드를 호출 하거나 호출 중일 때 예외가 발생한다.
     - 예외를 던지지 않는 스레드는 interrupt 상태가 지속되어 다음 코드를 수행할 수 없게 된다. 이를 방지하기 위해 `.interrupted()`를 사용한다.
- `Thread.interrupted()`
  - 스레드가 인터럽트 상태면 true를 반환하고, 해당 스레드의 인터럽트 상태를 false로 변경한다.
  - 스레드가 인터럽트 상태가 아니면 false를 반환하고, 해당 스레드의 인터럽트 상태를 변경하지 않는다.
